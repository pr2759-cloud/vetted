...
class SearchService {
   ...
   */
  async performSearch({ query, filters = {}, page = 1, limit = 20, sortBy = 'relevance', sortOrder = 'desc' }) {
    try {
      ...
      let products = await this.searchProducts(query, filters);

      // Apply sentiment-based ranking
  ...
  async searchProducts(query, filters) {
    try {
      // Mock product database - replace with actual database search
      ...
      let results = allProducts.filter(product => {
        // Basic text matching
        const titleMatch = this.calculateTextRelevance(product.name.toLowerCase(), searchTerms);
        const descriptionMatch = product.description ? 
          this.calculateTextRelevance(product.description.toLowerCase(), searchTerms) : 0;
        const categoryMatch = this.calculateCategoryRelevance(product.category, query);
        const brandMatch = product.brand ? 
          this.calculateTextRelevance(product.brand.toLowerCase(), searchTerms) : 0;
        const tagMatch = product.tags ? 
          this.calculateTagRelevance(product.tags, searchTerms) : 0;

        // Calculate overall relevance score
        const relevanceScore = 
          titleMatch * SEARCH_CONFIG.RELEVANCE_WEIGHTS.TITLE_MATCH +
          descriptionMatch * SEARCH_CONFIG.RELEVANCE_WEIGHTS.DESCRIPTION_MATCH +
          categoryMatch * SEARCH_CONFIG.RELEVANCE_WEIGHTS.CATEGORY_MATCH +
          brandMatch * SEARCH_CONFIG.RELEVANCE_WEIGHTS.BRAND_MATCH +
          tagMatch * SEARCH_CONFIG.RELEVANCE_WEIGHTS.TAG_MATCH;
  ...
  generateNoResultsResponse(query) {
    ...
    let response = `I couldn't find any products matching "${query}".`;
